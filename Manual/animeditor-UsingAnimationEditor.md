...
> 动画视图（左侧）显示了当前被选中游戏对象使用的动画，如果它的子对象也被动画控制，那么也会显示在这里。场景视图和层级视图显示在右侧，可以看到当前选中的游戏对象。
...
动画视图的左侧是动画属性列表。在一个新创建的动画剪辑中，是没有记录任何动画的，这个列表为空。当你开始让各种属性动起来时，参与动画的属性将显示在这里。如果动画控制了多个子对象，这个列表还会以层级的方式显示子对象的动画属性。在上面的例子中，机械手臂的各个部分都由同一个动画剪辑驱动，每个被它控制的游戏对象，按照在附加了动画组件的根元素中的层级关系显示。
...
每个属性可以折叠和展开，从而显示每一个关键帧的精确值。如果在关键帧之间移动磁头（红线），属性右侧的值域将显示补间值。值域可以直接编辑。如果磁头在某个关键帧上，修改值域将直接修改该关键帧的值。如果磁头在关键帧之间（此时值域显示的是补间值），修改值域将在磁头位置创建一个新的关键帧，值域的值即为该关键帧的值。
...
> 动画视图中展开属性列表，允许直接键入关键帧的值。在这张图中，因为磁头（红色）位于关键帧之间，所以显示的是补间值。在该位置键入一个新值，将创建一个新关键帧。
...
当使用曲线模式查看动画剪辑时，非常重要的一点是，每个属性值的范围有时会相差很大。举个例子，假如有一个同时旋转和弹跳的立方体。弹跳的 Y 坐标值可能介于 0 和 2 之间（也就是说，立方体在动画过程中弹跳 2 个单位），但是，旋转值介于 0 和 360 之间。同时查看这两条曲线时，位置动画曲线将非常难辨别，因为整个视图被放大，以显示 0 到 360。
...
> 同时选择立方体的位置动画和旋转动画，因为视图被放大，以显示旋转曲线的 0 到 360，位置曲线的 Y 轴值不可辨识。
...
点击动画视图左上角的下拉框，选择 Create New Clip，可以为选中的游戏对象创建一个新的动画剪辑。然后，会提示你把动画剪辑保存到 Assets 文件夹的某个位置。如果这个游戏对象已经附加了动画组件，并且指定了动画控制器，新的动画剪辑将作为状态，被添加到现有的动画控制器中。
...
如果这个游戏对象尚未绑定动画组件，那么，将自动进行下面的操作：
...
> 一个新的动画剪辑被创建，并保存为一个资源。动画剪辑作为默认状态，被自动添加到一个新的动画控制器中，该动画控制器也被保存为一个资源。动画控制器被分配给游戏对象上的动画组件。
...
在下面的图中，你可以看到一个没有动画的游戏对象。它只有一个简单的立方体，没有附加动画组件。动画视图、项目视图、检视视图挨个排列如下。
...
<!-- > After: After creating a new clip, you can see the new assets created in the project window, and the Animator Component assigned in the Inspector window (far right). You can also see the new clip assigned as the default state in the Animator Window -->
> 之后：创建一个新的动画剪辑后，你可以在项目视图中看到新资源文件，在检视视图（右侧）中看到附加的动画组件。你还可以在动画控制器视图中看到该动画剪辑被当作了默认状态。
...
一旦你保存了新动画剪辑，就可以开始为动画剪辑添加关键帧了。开始为选中的游戏对象编辑动画剪辑之前，需要先点击动画记录按钮，进入动画记录模式，此时，对动画对象的改变会被记录到动画剪辑。
...
![Add curves](https://docs.unity3d.com/uploads/Main/AnimationEditorAddCurves.png)
<!-- > Add curves -->
> 添加曲线
...
在**动画录制模式**中，红色的垂直线表示当前预览的是动画剪辑的哪一桢。检视视图和场景视图将显示东动画剪辑在该桢的状态。动画属性在该桢的值显示在属性名称的右侧。
...
在动画模式下，你可以在场景视图中移动、旋转或缩放游戏对象。如果位置、旋转、缩放属性在动画剪辑中不存在的话，这些操作将自动创建动画曲线；并且，自动在当前预览的桢上创建关键帧，以存储改变后的 Transform 值。
...
你也可以在检视视图中修改游戏对象的任意动画属性。如果需要的话，这些操作也会自动创建动画曲线，并且在当前预览的桢上创建动画曲线，以存储改变后的 Transform 值。
...
你可以使用添加关键桢按钮手动创建一个关键帧。这个操作将会为动画视图中当前选中的所有属性创建一个关键帧。也就是说，你可以有选择性地只为特定属性添加关键帧。
...
任意时候，在动画视图中点击播放按钮，都可以播放&重放动画。

![The Play button](https://docs.unity3d.com/uploads/Main/AnimationEditorPlayButton.png)
> The Play button
> 播放按钮
...
想要学习控制曲线模式的更多内容，请查阅 [使用动画曲线](https://docs.unity3d.com/Manual/animeditor-AnimationCurves.html) 一节。
